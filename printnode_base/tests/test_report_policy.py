...
class TestPrintNodeReport(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.action_method = self.env['printnode.action.method'].create({
            'name': 'SO Print',
    ...
    def test_get_default_action_printer(self):
        # getting printer from action button
        self.user.printnode_printer = False
        self.user.company_id.printnode_printer = False
        self.action_button.printer_id = self.printer.id
        printer = self.action_button.sudo(user=self.user)._get_action_printer()
        self.assertEqual(printer, self.printer)

        # getting printer from user
        self.action_button.printer_id = False
        self.user.printnode_printer = self.printer.id
        printer = self.action_button.sudo(user=self.user)._get_action_printer()
        self.assertEqual(printer, self.printer)

        # getting printer from company
        self.user.printnode_printer = False
        self.user.company_id.printnode_printer = self.printer.id
        printer = self.action_button.sudo(user=self.user)._get_action_printer()
        self.assertEqual(printer, self.printer)

        # raise if trere is no one printer is available
        self.user.printnode_printer = False
        self.user.company_id.printnode_printer = False
        with self.assertRaises(UserError):
            printer = self.action_button.sudo(user=self.user)._get_action_printer()
